...
public class ESSelectionCGP extends ESSelection {
	public int produce(final int min, final int max, final int start,
		...
		ArrayList<Individual> inds2 = state.population.subpops.get(subpopulation).individuals;
		...
		Individual parentInd = state.population.subpops.get(subpopulation).individuals.get(parent);
		...
		if (pos == 0) {
			...
			parentInd = state.population.subpops.get(subpopulation).individuals.get(0);
